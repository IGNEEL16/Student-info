    ...
    Private Sub Button6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button6.Click
        Try
            If cn.State = ConnectionState.Closed Then
                cn.Open()
            End If
            Dim _rows As Boolean
            cmd = New SqlCommand
            Dim qry As String = " Select * FROM Photos where  prn = '" & TextBox1.Text & "'"
            cmd.CommandText = qry
            cmd.Connection = cn
            dr = cmd.ExecuteReader
            _rows = dr.HasRows
            ...
            If _rows Then
                cmd.CommandText = " update Photos set image = '@image', name = '@name' where prn = @prn "
            Else
            ...
            If upd = False Then
                ...
                AddmisionForm.Show()
            Else
                AddmisionForm1.PictureBox1.Load(OpenFileDialog1.FileName)
            End If
            Me.Close()


        Catch ex As Exception
            MsgBox("Record Not Found." & ex.Message)
